...
export default function EditPostPage() {
    ...
    }, [user, isAdmin, loading, router, slug]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // We use the ORIGINAL slug from params for the doc reference, 
            // but we save the NEW slug in the data if they changed it.
            // Note: Changing slug without moving the doc ID creates a mismatch. 
            // For simplicity, we assume they don't change slug often, or if they do, 
            // they know it might break old links.
            // Ideally: Create new doc, delete old. But let's stick to update to keep it simple.
                ...
                <div className="glass-dark border border-white/10 rounded-2xl p-6 sm:p-10">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-300">Slug (Read Only)</label>
                                <input
                                    type="text"
                                    readOnly
                                    value={formData.slug}
                                    className="w-full bg-black/40 border border-white/5 rounded-lg px-4 py-3 text-gray-500 font-mono text-sm cursor-not-allowed"
                                    title="To change slug, delete and re-create post."
                                />
                            </div>
                        </div>
